...
maxcof <- all_combine %>% group_by(Year) %>% summarise(maxcof=max(CoF, na.rm=T)/1e6) %>% arrange(-Year)
maxcof %>% ggplot(aes(Year, maxcof))+geom_col(fill='coral2')+theme_bw(14,"serif")+
  labs(title="CoF for Gas Transmission Incidents",subtitle = "Max CoF by Year 1986 - 2019",y="Annual Max. CoF ($MM)")
